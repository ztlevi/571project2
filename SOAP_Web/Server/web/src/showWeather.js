$(document).ready(function() {
  //initial pull to find user's location
  //can't be done over "https" because getJSON function requres "http"
  $.getJSON("http://ip-api.com/json/?callback", function(json) {
    ...
    var key = "&APPID=267e66e22e84beceecb6d51085794fbf";
    var tempUnits = "&units=imperial";
    var urlToCall = baseURL + city + "," + countryCode + tempUnits + key;
    //inital pull to find weather at user's location
    $.getJSON(urlToCall, function(json) {
      var currentTemp = json.main.temp;
      var currentWeather = json.weather[0].main;

      document.getElementById("temperature").innerHTML = Math.round(currentTemp);
      ...
      //cases for image to appear based on weather type
      //old icons
      switchIconCase(currentWeather);
      skycons.play();
      /*function switchIconCase(currentWeather) {
        switch (json.weather[0].main) {
          ...
          case "Clouds":
            ...
            break;
          case "Thunderstorm":
            document.getElementById("weather-icon").innerHTML = '<img src="http://openweathermap.org/img/w/11d.png">';
            break;
          case "Drizzle":
            document.getElementById("weather-icon").innerHTML = '<img src="http://openweathermap.org/img/w/09d.png">';
            break;
      ...
      //click to convert C <--> F
      $(".tempType").on("click", function() {
        if (tempUnits == "&units=imperial") {
